...
# def loadModel(x_test):
#     modelPath = getModelPath.get()
#     if modelPath[-2:] == 'kl':
#         data_pkl = open(modelPath, "rb")
#         xgb_model_loaded = joblib.load(data_pkl)
...
#
#         # 以概率形式预测
#         y_probability = xgb_model_loaded.predict_proba(x_test)
#         # 归一化
#         min_number = np.min(y_probability)
#         max_number = np.max(y_probability)
#         normal_acc = MaxMinNormalization(y_probability, max_number, min_number)
#         return y_pred, normal_acc
#     elif modelPath[-2:] == 'h5':
#         H5model = load_model(modelPath)
#         x_data = np.reshape(x_test, (1, 28, 1))
#         predict = H5model.predict(x_data)
#         acc = list(np.array(predict).flatten())
#         normal_acc = np.reshape(acc, (1, 7))
#         y_pred = np.argmax(predict)  # 取最大值的位置
#         return y_pred, normal_acc
...
def application():
    start = time.time()
    rock = ['黑色煤', '深灰色粉砂质泥岩', '深灰色泥岩', '灰黑色泥岩', \
            '灰色细砂岩', '灰色泥质粉砂岩', '浅灰色细砂岩']
...
rocks = ['黑色煤', '深灰色粉砂质泥岩', '深灰色泥岩', '灰黑色泥岩', \
         '灰色细砂岩', '灰色泥质粉砂岩', '浅灰色细砂岩']

preValueout = [0, 0, 0, 0, 0, 0, 0]
for i in range(7):
...
Label(frm22, text="网络预测所用时间:", anchor=NW, fg="white", font=ft3, bg=_from_rgb((9, 66, 119))).place(x=0, y=330)
label3 = Label(frm22, anchor=NW, fg="red", font=ft3, bg=_from_rgb((9, 66, 119)))
label3.place(x=130, y=360)
